<template>
  <add-form class="w705" :isForm="true" v-on:formSubmit="save">
    ...
    <div  class="button-area" slot="buttonArea">
      ...
      <router-link to="/content/knowledge" class="btn btn-red" replace>取消</router-link>
    </div>
...
import FileUpload from 'vue-upload-component'
    export default{
        ...
        computed: {
            isEdit () {
                return this.$route.name == 'editKnowledge';
            },
            classifyArr () {
        ...
        methods: {
          save () {
               ...
               const url = this.isEdit ? '/api/subwayKnowledge/update' : '/api/subwayKnowledge/insert';
               this.$http.post(url,{obj:JSON.stringify(this.formObj)})
               .then( res => {
                   const msg = res.body
                   if(msg.success){
                       this.$message.success({message: '操作成功!'});
                       this.$router.replace('/content/knowledge');
                   }else{
          ...
          getView () {
               this.$http.get('/api/subwayKnowledge/view',{params:{id: this.$route.params.id}})
               .then( (res) =>{
                  const msg = res.body;
                  if(msg.success){
