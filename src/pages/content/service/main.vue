  ...
  <div>
    ...
    <panel-wrap class="panelWrap2">
      ...
      <ul class="action-list">
        <li><router-link to="/content/service/add" class="btn btn-blue">新建内容</router-link></li>
      </ul>
      ...
      <ui-table :headerArray="headerArray" :style="{minHeight:tableMinHeight}">
        ...
        <tr v-for="data in tableData">
          ...
          <td class="action-td">
            <router-link class="blue-fontIcon" title="编辑" :to="{ name: 'editService', params: { id: data.id }}"><i class="iconfont icon-modify"></i></router-link>
            <router-link class="blue-fontIcon" title="查看详情" :to="{ name: 'serviceDetail', params: { id: data.id }}"><i class="iconfont icon-info"></i></router-link>
          </td>
        </tr>
...
import pagination from '../../../components/pagination'
  export default{
      ...
      computed: {
         ...
         classifyArr () {
          return this.$store.state.classify.serviceArr;
         }
      },
      components:{
        panelWrap,
      ...
      methods: {
        getAdvList (page,obj) {
           const searchObj = obj || {};
           page = page || 1;
           this.$http.get('/api/serviceExplain/getLists',{params:{pageNum: page, pageSize: this.pageSize, obj: JSON.stringify(searchObj)}})
             .then( (res) =>{
                const msg = res.body;
                if(msg.success){
        ...
        switchChange (data) {
          ...
          if(!this.firstLoad){
           this.$http.post('/api/serviceExplain/updateState',{obj:JSON.stringify({id:data.id,state:data.state,classify:data.classify})})
             .then( res => {
                 const msg = res.body
                 if(msg.success){
