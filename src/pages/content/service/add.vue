  ...
  <add-form class="w705" :isForm="true" v-on:formSubmit="save">
    ...
    <div slot="formBody" class="form-body">
      ...
      <dl class="dl2">
        <dt>分类信息：</dt>
        <dd>
          <select v-model="formObj.classify" class="ipt ipt-medium">
    ...
    <div  class="button-area" slot="buttonArea">
      ...
      <router-link to="/content/service" class="btn btn-red" replace>取消</router-link>
    </div>
...
import FileUpload from 'vue-upload-component'
    export default{
        ...
        computed: {
            isEdit () {
                return this.$route.name == 'editService';
            },
            classifyArr () {
        ...
        methods: {
          save () {
               ...
               const url = this.isEdit ? '/api/serviceExplain/update' : '/api/serviceExplain/insert';
               this.$http.post(url,{obj:JSON.stringify(this.formObj)})
               .then( res => {
                   const msg = res.body
                   if(msg.success){
                       this.$message.success({message: '操作成功!'});
                       this.$router.replace('/content/service');
                   }else{
          ...
          getView () {
               this.$http.get('/api/serviceExplain/view',{params:{id: this.$route.params.id}})
               .then( (res) =>{
                  const msg = res.body;
                  if(msg.success){
