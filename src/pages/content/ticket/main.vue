  ...
  <div>
    ...
    <panel-wrap class="panelWrap2">
      ...
      <ul class="action-list">
        <li><router-link to="/content/ticket/add" class="btn btn-blue">新建内容</router-link></li>
      </ul>
      <ui-table :headerArray="headerArray" :style="{minHeight:tableMinHeight}">
        ...
        <tr v-for="data in tableData">
          ...
          <td class="action-td">
            <router-link class="blue-fontIcon" title="编辑" :to="{ name: 'editTicket', params: { id: data.id }}"><i class="iconfont icon-modify"></i></router-link>
            <router-link class="blue-fontIcon" title="查看详情" :to="{ name: 'ticketDetail', params: { id: data.id }}"><i class="iconfont icon-info"></i></router-link>
          </td>
        </tr>
  ...
  export default{
      ...
      methods: {
        ...
        getAdvList (page,obj) {
           const searchObj = obj || {};
           page = page || 1;
           this.$http.get('/api/ticketKnow/getLists',{params:{pageNum: page, pageSize: this.pageSize, obj: JSON.stringify(searchObj)}})
             .then( (res) =>{
                const msg = res.body;
                if(msg.success){
        ...
        switchChange (data) {
          ...
          if(!this.firstLoad){
           this.$http.post('/api/ticketKnow/updateState',{obj:JSON.stringify({id:data.id,state:data.state,classify:data.classify})})
             .then( res => {
                 const msg = res.body
                 if(msg.success){
